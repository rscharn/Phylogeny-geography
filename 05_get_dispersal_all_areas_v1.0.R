...
gc(verbose = FALSE)
###############INPUTS############################
wd="C:\\Users\\Ruud\\Desktop\\R working Directory\\scripts fernanda\\LASTEST VERSIONS\\Ferns"
#wd="C:\\Users\\Sidonie\\Dropbox\\Carvalho_et_al_Neotropical_biome_shifts\\Data_and_analyses\\Analyses\\final_Biogeobears_analyses\\birds"
...
div.plots <-function(binsize = 7, area = area, wd = getwd()){
  setwd(wd)
  ...
  pdf(file="Dispersal_plots1.pdf",paper = "a4")
  par(mfrow=c(2,2))
  combinations<-combs(area,2)
  
  for (r in 1:length(combinations[,1])){
    ...
    for (o in 1:number){
      ...
      for (i in 1:length(tr$edge[,1])){
        if ((areaperbranch[tr$edge[i,2],6] != areapernode[tr$edge[i,2],6])&(areaperbranch[tr$edge[i,2],6] != 0)&(areapernode[tr$edge[i,2],6] != 0)&
          ...
          if (areaperbranch[tr$edge[i,2],7] == FALSE){
            areaperbranch[tr$edge[i,2],7] = TRUE
    ...
    fivemilgaps = matrix(fivemilgaps[,c(1,5,2,3,6,8,9)],nrow = length(fivemilgaps[,1]),ncol=7)
    fivemilgaps[,1] <- data.frame(fivemilgaps)[,1]-2.5
    ...
    plot(fivemilgaps[,"Age"],fivemilgaps[,"Rel.tot"],type = 'l',xlab = "Ma",ylab = "Relative number of dispersals",main= paste("Relative dispersal ",combinations[r,1],"<->",combinations[r,2],sep=""))
    plot(fivemilgaps[,"Age"],fivemilgaps[,"Abs.tot"],type = 'l',xlab = "Ma",ylab = "Number of dispersal events",main=paste("Absolute dispersal ",combinations[r,1],"<->",combinations[r,2]," total: ",sum(fivemilgaps[,6]),sep=""))
...
div.plots(wd = wd, binsize = binsize, area = area)
